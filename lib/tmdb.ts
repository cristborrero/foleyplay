...
export async function fetchTMDB<T>(endpoint: string, params: Record<string, string> = {}, language = 'es-MX'): Promise<T> {
  if (!TMDB_API_KEY) {
...
export function extractRating(detail: TMDBDetail): string {
  ...
  if (isTv) {
    const usRating = detail.content_ratings?.results?.find(
