      ...
      <div className="overflow-y-auto h-full [&::-webkit-scrollbar]:hidden bg-fp-black" style={{ scrollbarWidth: 'none' }}>
        <div className="relative h-[40vh] w-full">
          ...
          <div className="absolute bottom-0 left-0 p-6 w-full">
            ...
            <DetailActions tmdbId={Number(id)} mediaType="movie" title={title} posterPath={movie.poster_path || ''} />
          </div>
        </div>
        <div className="px-6 py-4">
          <TVMovieDetail tmdbId={Number(id)} imdbId={movie.imdb_id} similar={movie.similar?.results || []} />
        </div>
      </div>
    );
  } catch {
